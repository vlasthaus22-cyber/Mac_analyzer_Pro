...
Если Python не определяется автоматически, откройте расположенный рядом файл
`PYTHON_PATH.cmd` и впишите полный путь после знака `=`:

```cmd
set "MAC_ANALYZER_PYTHON=C:\Users\User\AppData\Local\Programs\Python\Python312\python.exe"
```

Сохраняйте кавычки как в примере. Администраторские права не требуются. Чтобы
узнать путь автоматически, выполните в обычной консоли `where python` или
...
- Обычная загрузка DDIO теперь применяет профиль `Column3/Column2/Column10/Column9`, как и массовый режим.
- Пустой IP устройства заполняется из DDIO по точному MAC независимо от IP коммутатора.

### Исправления v1.0.72

- Исправлена самодиагностика автономного HTML: загрузка контроллера больше не
  считается ошибкой, пока большая IndexedDB продолжает восстанавливаться.
- Аналитика пересчитывает устаревшие/неполные агрегаты по всем строкам Final,
  а не по странице из 50 устройств, и сохраняет все метаданные снимков.
- Исправлен расчёт MAC, которые исчезают и возвращаются: поток читает реальные
  `device`-части каждого снимка.
- SmartRoom поддерживает поле «Тип устройства» (`Тип модели`, `Тип`), а KPI
  «Изменения кодеков» открывает устройства типа `Video Conference`.
- История MAC автоматически делится на безопасные листы; полный XLSX допускает
  до 2 ГБ и содержит последний Final, накопительную базу, выбранные изменения,
  аналитику, источники и данные графиков.
- Добавлены CSV для всех устройств и изменений выбранного периода, а локальные
  архивы можно выгружать в файл и восстанавливать в другом браузере.
...
В инженерном режиме откройте **Автоматизация → Массовое обогащение** и
подключите три папки: основные выгрузки, SmartRoom и DDIO. Поддерживаются CSV,
TSV, TXT, JSON, XLS/XLSX/XLSM. Каждый основной файл становится отдельным
якорем цикла:

`△ Main + △ SmartRoom + △ DDIO → □ Final`.

Режим **Ближайшие даты** назначает SmartRoom и DDIO ближайшему основному файлу
в пределах настраиваемого допуска (по умолчанию 24 часа). Режим **Одинаковая
календарная дата** требует совпадения дня. Даже если два основных файла имеют
одинаковую дату, они остаются двумя отдельными Final. Перед запуском можно
переназначить SmartRoom/DDIO, отключить отдельный цикл и проверить список
неназначенных файлов.

При запуске через `START_MAC_ANALYZER.cmd` можно раскрыть блок **Точные даты
создания CSV через Python** и указать полные пути Windows. В этом режиме дата
определяется по имени файла, затем по свойствам XLSX и по дате создания файла
Windows. В автономном HTML применяется та же дата из имени, свойства XLSX и
доступный браузеру `lastModified`. Циклы выполняются последовательно и используют
тот же атомарный конвейер, что обычное обогащение: DDIO не создаёт устройства,
модели восстанавливаются из истории/правил, а ошибка одной группы не повреждает
последний успешный Final.
