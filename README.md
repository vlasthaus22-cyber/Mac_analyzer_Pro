...
Полный комплект v1.0.71: `MAC-Analyzer-v1.0.71-Full.zip` в GitHub Releases.
...
[`RELEASE_NOTES_v1.0.71.md`](RELEASE_NOTES_v1.0.71.md).

## Что изменилось в v1.0.71

- DDIO заполняет пустой IP устройства по точному MAC независимо от смены IP
  коммутатора и не заменяет уже известное значение.
- В массовом обогащении у каждого цикла есть кнопка «Проверить колонки».
  Встроены пресеты Main (`CallingStarionID`, `NasIP`, `NasPortID`,
  `B_ReceiptTime`), SmartRoom и DDIO (`Column2/3/9/10`).
- Analytics пересчитывает KPI и журнал при каждом изменении диапазона/фильтра;
  отдельный график показывает MAC, которые исчезают и возвращаются.
- Верхний поиск работает по накопительному реестру всех Final.
- В настройках можно создавать, загружать и удалять именованные локальные
  архивы состояния. Для переноса между браузерами используется полная MADB.
