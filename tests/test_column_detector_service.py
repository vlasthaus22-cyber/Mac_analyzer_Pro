...
def test_requested_smartroom_headers_are_detected_exactly():
    result = detect(["MAC", "IP адрес", "Производитель", "Модель", "Тип модели", "Адрес комнаты", "Название комнаты", "ID комнаты"])
    assert result["mapping"]["mac"] == 0
    assert result["mapping"]["vendor"] == 2
    assert result["mapping"]["model"] == 3
    assert result["mapping"]["deviceType"] == 4
    assert result["mapping"]["address"] == 5
    assert result["mapping"]["room"] == 6
    assert result["mapping"]["smartroomId"] == 7
