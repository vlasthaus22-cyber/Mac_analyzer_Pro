...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 106
    assert status["registry"]["total"] == 106
    ...
    assert "PARITY_REGISTRY.md requirements: 106/106." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 106
