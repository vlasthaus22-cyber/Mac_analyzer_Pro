...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 113
    assert status["registry"]["total"] == 113
    ...
    assert "PARITY_REGISTRY.md requirements: 113/113." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 113
