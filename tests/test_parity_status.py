...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 60
    assert status["registry"]["total"] == 60
    ...
    assert "PARITY_REGISTRY.md requirements: 60/60." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 60
