...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 118
    assert status["registry"]["total"] == 118
    ...
    assert "PARITY_REGISTRY.md requirements: 118/118." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 118
