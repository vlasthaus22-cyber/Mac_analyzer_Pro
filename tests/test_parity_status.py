...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 53
    assert status["registry"]["total"] == 53
    ...
    assert "PARITY_REGISTRY.md requirements: 53/53." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 53
