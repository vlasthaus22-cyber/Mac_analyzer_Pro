...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 61
    assert status["registry"]["total"] == 61
    ...
    assert "PARITY_REGISTRY.md requirements: 61/61." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 61
