...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 71
    assert status["registry"]["total"] == 71
    ...
    assert "PARITY_REGISTRY.md requirements: 71/71." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 71
