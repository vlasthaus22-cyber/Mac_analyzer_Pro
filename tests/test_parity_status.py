...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 109
    assert status["registry"]["total"] == 109
    ...
    assert "PARITY_REGISTRY.md requirements: 109/109." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 109
