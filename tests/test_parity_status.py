...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 52
    assert status["registry"]["total"] == 52
    ...
    assert "PARITY_REGISTRY.md requirements: 52/52." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 52
