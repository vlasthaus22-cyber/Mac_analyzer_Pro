...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 59
    assert status["registry"]["total"] == 59
    ...
    assert "PARITY_REGISTRY.md requirements: 59/59." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 59
