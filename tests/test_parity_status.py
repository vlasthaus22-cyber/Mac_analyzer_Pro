...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 51
    assert status["registry"]["total"] == 51
    ...
    assert "PARITY_REGISTRY.md requirements: 51/51." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 51
