...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 64
    assert status["registry"]["total"] == 64
    ...
    assert "PARITY_REGISTRY.md requirements: 64/64." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 64
