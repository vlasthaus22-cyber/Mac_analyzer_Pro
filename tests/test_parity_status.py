...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 84
    assert status["registry"]["total"] == 84
    ...
    assert "PARITY_REGISTRY.md requirements: 84/84." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 84
