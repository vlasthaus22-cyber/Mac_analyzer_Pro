...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 100
    assert status["registry"]["total"] == 100
    ...
    assert "PARITY_REGISTRY.md requirements: 100/100." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 100
