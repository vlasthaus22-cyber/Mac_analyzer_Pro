...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 103
    assert status["registry"]["total"] == 103
    ...
    assert "PARITY_REGISTRY.md requirements: 103/103." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 103
