...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 50
    assert status["registry"]["total"] == 50
    ...
    assert "PARITY_REGISTRY.md requirements: 50/50." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 50
