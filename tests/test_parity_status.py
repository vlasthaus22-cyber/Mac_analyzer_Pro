...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 67
    assert status["registry"]["total"] == 67
    ...
    assert "PARITY_REGISTRY.md requirements: 67/67." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 67
