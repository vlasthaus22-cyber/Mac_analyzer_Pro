...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 123
    assert status["registry"]["total"] == 123
    ...
    assert "PARITY_REGISTRY.md requirements: 123/123." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 123
