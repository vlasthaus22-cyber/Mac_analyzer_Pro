...
def test_parity_registry_and_pyqt_equivalents_are_complete():
    ...
    assert status["registry"]["checked"] == 55
    assert status["registry"]["total"] == 55
    ...
    assert "PARITY_REGISTRY.md requirements: 55/55." in audit
...
def test_parity_status_endpoint():
    ...
    assert payload["registry"]["checked"] == 55
