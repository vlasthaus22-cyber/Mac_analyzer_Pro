...
выбирает папку вручную в каждом новом браузере. Если прямой доступ к папке не
поддерживается, используйте открытие и сохранение отдельного файла MADB.
...
когда база в папке новее кэша. Неизменённая база больше не переписывается каждую
минуту, а повторный импорт одного исходного файла обновляет его архивную копию.
