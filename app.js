(() => {
  ...
  const defaultColumnWidths = {macFormatted:180,oui:110,vendor:150,model:150,deviceType:170,ip:130,address:200,room:120,smartroomId:140,switchIp:150,switchPort:100,authenticationTime:210,hostname:160,serialNumber:160,deviceId:150,deviceName:180,source:150};
  ...
  const PresenceChurn = window.MacAnalyzerPresenceChurn;
  ...
  const ColumnPresets = window.MacAnalyzerColumnPresets;
  ...
  if(!ColumnPresets)throw new Error("Модуль frontend/column-presets.js не загружен");
  ...
  for(const keyName of ["visibleColumns","columnOrder"]){
    ...
    if(!columns.includes("authenticationTime")){
      const portIndex=columns.indexOf("switchPort");
      columns.splice(portIndex>=0?portIndex+1:columns.length,0,"authenticationTime");
    }
    if(!columns.includes("deviceType")){
      const modelIndex=columns.indexOf("model");
      columns.splice(modelIndex>=0?modelIndex+1:columns.length,0,"deviceType");
    }
  ...
  function persistLatestBrowserState(){
    ...
    const revision=browserStateSaveRevision,savedAt=browserStatePendingSavedAt||new Date().toISOString();
    browserStateQueuedRevision=revision;
    // Build the compact payload only when this queued write starts. This avoids
    // retaining another large structured-clone candidate behind an active save.
    browserStateSavePromise=browserStateSavePromise.catch(()=>{}).then(()=>writeBrowserStateRecord(browserStateSnapshot(),savedAt)).then(()=>{browserStatePersistedRevision=Math.max(browserStatePersistedRevision,revision);if(browserStateSaveRevision>browserStateQueuedRevision)armBrowserStateSave(0);});
  ...
  async function browserArchiveRecords(){const db=await openBrowserStateDb();return new Promise((resolve,reject)=>{const tx=db.transaction(browserStateStoreName,"readonly"),request=tx.objectStore(browserStateStoreName).getAll();request.onsuccess=()=>resolve((request.result||[]).filter((item)=>String(item.id||"").startsWith("archive:")));request.onerror=()=>reject(request.error||new Error("Не удалось прочитать архивы"));tx.oncomplete=()=>db.close();});}
  async function writeBrowserArchive(name){const db=await openBrowserStateDb(),id="archive:"+crypto.randomUUID(),record={id,name:String(name||"").trim()||`Архив ${new Date().toLocaleString("ru-RU")}`,createdAt:new Date().toISOString(),state:compactBrowserState(state)};return new Promise((resolve,reject)=>{const tx=db.transaction(browserStateStoreName,"readwrite");tx.objectStore(browserStateStoreName).put(record);tx.oncomplete=()=>{db.close();resolve(record);};tx.onerror=()=>{const error=tx.error;db.close();reject(error||new Error("Не удалось создать архив"));};});}
  async function deleteBrowserArchive(id){const db=await openBrowserStateDb();return new Promise((resolve,reject)=>{const tx=db.transaction(browserStateStoreName,"readwrite");tx.objectStore(browserStateStoreName).delete(String(id));tx.oncomplete=()=>{db.close();resolve(true);};tx.onerror=()=>{const error=tx.error;db.close();reject(error||new Error("Не удалось удалить архив"));};});}
  async function renderArchiveList(){const root=$("#archiveList");if(!root)return;try{const records=(await browserArchiveRecords()).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));root.innerHTML=records.length?records.map((item)=>`<div class="mapping-row"><span><strong>${esc(item.name)}</strong><small>${esc(new Date(item.createdAt).toLocaleString("ru-RU"))} · Final: ${Number(item.state?.snapshots?.length||0)}</small></span><span><button class="button secondary" type="button" data-load-archive="${esc(item.id)}">Загрузить</button> <button class="button secondary" type="button" data-export-archive="${esc(item.id)}">В файл</button> <button class="button secondary" type="button" data-delete-archive="${esc(item.id)}">Удалить</button></span></div>`).join(""):'<p class="muted">Сохранённых архивов нет.</p>';}catch(error){root.innerHTML=`<p class="muted">Архивы недоступны: ${esc(error.message)}</p>`;}}
  async function loadBrowserArchive(id){const records=await browserArchiveRecords(),record=records.find((item)=>item.id===id);if(!record)throw new Error("Архив не найден");state=normalizeRestoredState(record.state||{});save({immediate:true});await flushBrowserStateSave();location.reload();}
  async function exportBrowserArchive(id){
    const record=(await browserArchiveRecords()).find((item)=>item.id===id);if(!record)throw new Error("Архив не найден");
    const archiveState=normalizeRestoredState(record.state||{}),currentId=String(archiveState.resultBrowserSnapshotId||archiveState.snapshots?.at(-1)?.id||archiveState.snapshots?.[0]?.id||"");
    const streamSnapshotRows=async(snapshot,accept)=>{const metadata=await BrowserSnapshots?.streamSnapshot?.(snapshot.id,async(kind,rows)=>{if(kind==="device")await accept(rows);});if(!metadata)await accept(snapshot.devices||[]);};
    const result=await FullJsonReport.createReport({state:archiveState,streamCurrentRows:async(accept)=>{if(currentId){const metadata=await BrowserSnapshots?.streamSnapshot?.(currentId,async(kind,rows)=>{if(kind==="device")await accept(rows);});if(metadata)return;}await accept(archiveState.devices||[]);},streamInventoryRows:streamAllDeviceRows,streamInvalidRows:async(accept)=>accept(archiveState.invalid||[]),streamSnapshotRows,analyticsPayload:async()=>currentId&&BrowserSnapshots?.aggregate?analysisDashboardAggregatePayload(await BrowserSnapshots.aggregate(currentId,{limit:200})):analysisDashboardLocalPayload(archiveState.devices||[])});
    const safeName=String(record.name||"archive").replace(/[\\/:*?"<>|]+/g,"-").trim()||"archive";deliverDownload(`${safeName}.macarchive.json`,result.blob);toast("Архив выгружен в файл.");return result;
  }
  async function importBrowserArchiveFile(file){
    if(!file)throw new Error("Файл архива не выбран");const payload=JSON.parse(await file.text());if(payload?.format!=="mac-analyzer-full-json"||!Array.isArray(payload.snapshots))throw new Error("Это не полный архив MAC Analyzer Pro");
    const processId=beginProcess("Импорт архива","Восстановление Final и накопительной базы",5),metadata=[];
    try{
      for(let index=0;index<payload.snapshots.length;index++){const entry=payload.snapshots[index]||{},meta={...(entry.metadata||{}),id:String(entry.metadata?.id||`import-${crypto.randomUUID()}`),devices:entry.devices||[],invalid:[],browserStored:true,backendStored:false};const saved=await BrowserSnapshots.save(meta);metadata.push({...saved,devices:[],invalid:[],browserStored:true});updateProcess(processId,10+Math.round((index+1)/Math.max(1,payload.snapshots.length)*70),`Final ${index+1} из ${payload.snapshots.length}`);}
      if(Array.isArray(payload.allDevices)&&BrowserSnapshots?.mergeDeviceHistoryRows){for(let offset=0;offset<payload.allDevices.length;offset+=1000){await BrowserSnapshots.mergeDeviceHistoryRows(payload.allDevices.slice(offset,offset+1000),"archive-import");}}
      const current=Array.isArray(payload.currentDevices)?payload.currentDevices:[],latest=metadata.at(-1)||metadata[0]||null;
      state=normalizeRestoredState({...state,...(payload.workspace||{}),snapshots:metadata,movementHistory:Array.isArray(payload.movementHistory)?payload.movementHistory:[],devices:current.slice(0,resultPageSize),invalid:Array.isArray(payload.invalidRows)?payload.invalidRows.slice(0,resultPageSize):[],resultBrowserSnapshotId:latest?.id||"",resultDeviceCount:Number(latest?.deviceCount||current.length),resultInvalidCount:Number(latest?.invalidCount||payload.invalidRows?.length||0),lastAnalysis:latest?.fileCreatedAt||latest?.createdAt||payload.exportedAt||new Date().toISOString()});
      save({immediate:true});await flushBrowserStateSave();await writeBrowserArchive(file.name.replace(/\.macarchive\.json$|\.json$/i,""));finishProcess(processId,`Восстановлено Final: ${metadata.length}`);toast("Архив восстановлен. Программа будет перезагружена.");location.reload();
  ...
  let analyticsRenderRevision=0,movementRenderRevision=0,dashboardDetailRenderRevision=0,dashboardChangesVisibleLimit=100,analyticsRenderCache={signature:"",at:0},historyRenderCache={signature:"",at:0};
  let statisticsPanelPromise=null,statisticsPanelPromiseKey="",browserFleetRefreshPromise=null,browserFleetRefreshKey="",presenceChurnCache={key:"",data:{snapshotCount:0,total:0,items:[]}};
  ...
  function compactAnalysisAutosaveState(){
    ...
    return {
      ...
      devices:[],
      invalid:[],
      snapshots:(state.snapshots||[]).map((snapshot)=>({...snapshot,devices:[]})),
  ...
  const fileDateInfoCache=new WeakMap();
  ...
  async function prepareBatchGroup(group,groupIndex,totalGroups){
    ...
    const overrides=batchMappingOverrides.get(String(group.id))||{};
    const primary=await batchFileRecord(group.primary,"primary",progress);if(overrides.primary)primary.mapping={...primary.mapping,...overrides.primary};state.files=[primary];
    if(group.smartroom){const smartroom=await batchFileRecord(group.smartroom,"smartroom",progress);if(overrides.smartroom)smartroom.mapping={...smartroom.mapping,...overrides.smartroom};state.files.push(smartroom);}
    if(group.ddio){const ddio=await batchFileRecord(group.ddio,"ddio",progress);ddio.mapping={...DdioOverlay.detectMapping(ddio.headers||[]),...ColumnPresets.mappingForRole(ddio.headers||[],"ddio"),...(overrides.ddio||{})};const validation=DdioOverlay.validateMapping(ddio.mapping||{});if(!validation.valid)throw new Error(`DDIO «${ddio.name}»: не определены четыре колонки MAC/IP резервации и аренды`);state.ddioFile=ddio;}
  ...
  function batchMappingFields(role){return role==="ddio"?[["reservationMac","MAC резервации"],["reservationIp","IP резервации"],["leaseMac","MAC аренды"],["leaseIp","IP аренды"]]:fieldList;}
  function batchMappingSection(file,role){if(!file)return"";const options=(selected)=>['<option value="">Не использовать</option>',...(file.headers||[]).map((header)=>`<option value="${header.index}"${Number(selected)===Number(header.index)&&selected!==""?" selected":""}>${esc(mappingOptionLabel(header))}</option>`)].join("");return `<fieldset data-batch-mapping-role="${role}"><legend>${role==="primary"?"Основной файл":role==="smartroom"?"SmartRoom":"DDIO"} · ${esc(file.name)}</legend><div class="mapping-grid">${batchMappingFields(role).map(([field,label])=>`<label>${esc(label)}<select data-batch-map="${field}">${options(file.mapping?.[field])}</select></label>`).join("")}</div></fieldset>`;}
  async function reviewBatchMappings(groupId){
    if(batchRunActive)return;const group=batchPlan?.groups?.find((item)=>String(item.id)===String(groupId));if(!group)return;
    const token=UiFeedback?.start("Чтение заголовков файлов…");
    try{await prepareBatchGroup(group,0,batchPlan.groups.length);batchMappingReviewContext={group,files:{primary:state.files.find((file)=>file.role==="primary"),smartroom:state.files.find((file)=>file.role==="smartroom"),ddio:state.ddioFile}};$("#batchMappingReviewBody").innerHTML=[batchMappingSection(batchMappingReviewContext.files.primary,"primary"),batchMappingSection(batchMappingReviewContext.files.smartroom,"smartroom"),batchMappingSection(batchMappingReviewContext.files.ddio,"ddio")].join("");$("#batchMappingReviewDialog").showModal();}
    catch(error){UiFeedback?.showError(error);toast("Не удалось проверить колонки: "+error.message);}finally{UiFeedback?.stop(token);}
  }
  function applyBatchMappingReview(){if(!batchMappingReviewContext)return;const result={};$$('[data-batch-mapping-role]').forEach((section)=>{const mapping={};section.querySelectorAll('[data-batch-map]').forEach((select)=>{mapping[select.dataset.batchMap]=select.value===""?"":Number(select.value);});result[section.dataset.batchMappingRole]=mapping;});batchMappingOverrides.set(String(batchMappingReviewContext.group.id),result);batchMappingReviewContext.group.mappingChecked=true;batchMappingReviewContext=null;$("#batchMappingReviewDialog").close();renderBatchPlan();toast("Сопоставление колонок сохранено для этого цикла.");}
  ...
  function localAutoMapping(headers){const role=arguments[1]||"primary",names=ColumnPresets.headerNames(headers),find=(patterns,excluded=new Set())=>{const index=names.findIndex((header,column)=>!excluded.has(column)&&patterns.some(pattern=>pattern.test(String(header).toLowerCase())));return index>=0?index:"";},secondaryPatterns=[/secondary.*mac/,/additional.*mac/,/mac.*(?:2|secondary|additional)/,/мак.*(?:2|дополн|втор)/,/дополн.*мак/,/втор.*интерфейс/],secondaryMac=find(secondaryPatterns),mac=find([/mac/,/мак/,/callingstari?onid/],new Set(secondaryMac===""?[]:[secondaryMac])),generic={mac,secondaryMac,vendor:find([/vendor/,/производ/,/вендор/]),model:find([/model/,/модель/]),deviceType:find([/device.*type/,/model.*type/,/тип.*модел/,/^тип$/]),ip:find([/^ip$/,/^ip[ _-]*(?:address|адрес)$/,/адрес ip/]),address:find([/address/,/адрес/]),room:find([/room/,/помещ/,/кабин/,/название комнаты/,/наименование локации/]),smartroomId:find([/smart.?room.*id/,/id.*smart.?room/,/ид.*smart.?room/,/id комнаты/]),switchIp:find([/switch.*ip/,/коммутатор.*ip/,/^nasip$/]),switchPort:find([/port/,/порт/,/^nasportid$/]),authenticationTime:find([/authentication.*time/,/auth.*time/,/last.*auth/,/receipt.*time/,/время.*аутентификац/,/время.*авторизац/]),hostname:find([/host.?name/,/имя хоста/,/dns name/]),serialNumber:find([/serial/,/серийн/]),deviceId:find([/device.*id/,/id.*device/,/идентификатор.*устрой/]),deviceName:find([/device.*name/,/название.*устрой/,/наименование.*устрой/])};return ColumnPresets.mappingForRole(names,role,generic);}
  ...
  const unknownModelValues=new Set(["","unknown","not found","n/a","none","null","не определено","неизвестно","не указано"]);
  function isKnownModelValue(value){return!unknownModelValues.has(String(value??"").trim().toLowerCase());}
  ...
  async function applyLocalVendorModelMappingsToCurrentResult(source="local-vendor-model-rules"){
    ...
    try{
      const result=await updateCurrentBrowserSnapshot("Автоопределение производителей и моделей",source,(device)=>{
        ...
        if(!isKnownModelValue(device.model)&&model){changes.push(["model",device.model||"",model]);device.model=model;device.modelSource="prefix";}
  ...
  async function localAnalyzeFiles(fields,strategy,onProgress=()=>{}){
    ...
    try{
      ...
      // DDIO fills only a blank device IP by an exact MAC match. This is a
      // source-recovery rule and is deliberately independent from both the
      // SmartRoom IP checkbox and switch-IP change diagnostics.
  ...
  async function localAnalyzeFilesToSnapshot(fields,strategy,source,createdAt,onProgress=()=>{}){
    ...
    try{
      ...
      await BrowserSnapshots.transformEnrichmentRows(jobId,(device)=>{
        ...
        if(!isKnownModelValue(device.model)&&model){device.model=model;device.modelSource="prefix";changed=true;}
  ...
  function updateResultPager(total=0,page=1,pages=1){const status=$("#resultPageStatus"),count=$("#resultCount"),previous=$("#resultPreviousPageButton"),next=$("#resultNextPageButton"),size=$("#resultPageSizeSelect");page=Math.max(1,Math.min(Number(page)||1,Math.max(1,Number(pages)||1)));const start=total?(page-1)*resultPageSize+1:0,end=Math.min(Number(total)||0,page*resultPageSize);if(status)status.textContent=`${page} / ${pages}`;if(count)count.textContent=`Загружено ${Number(total||0).toLocaleString("ru-RU")} · показано ${start}–${end}`;if(previous)previous.disabled=page<=1;if(next)next.disabled=page>=pages;if(size)size.value=String(resultPageSize);resultPage=page;}
  ...
  function localSearchText(item){if(!item||typeof item!=="object")return "";if(localSearchTextCache.has(item))return localSearchTextCache.get(item);const text=[item.mac,item.macFormatted,item.vendor,item.model,item.deviceType,item.ip,item.address,item.room,item.smartroomId,item.switchIp,item.switchPort,item.authenticationTime,item.hostname,item.serialNumber,item.deviceId,item.deviceName,item.source].map((value)=>String(value||"")).join(" ").toLowerCase();localSearchTextCache.set(item,text);return text;}
  ...
  async function clientFileRecord(file,fileCreatedAt,onProgress=()=>{}){
    ...
    const fileRecord={id:crypto.randomUUID(),name:file.name,sheet:data.sheet||"",rows:previewRows,headers,mapping:localAutoMapping(rows[0]),createdAt:fileCreatedAt,fileDateSource:fileInfoDateSource(file),fileLastModified:file.lastModified||0,sourceBytes:file.size||0,clientImported:true,rowCount,rowsComplete:data.truncated!==true&&rowCount<=workspacePreviewDataRows};
  ...
  async function backendFileRecord(file,fileCreatedAt,onProgress=()=>{}){
    ...
    if(rows.length<2)throw new Error("файл не содержит строк данных");
    const headers=rows[0].map((name,index)=>({name:String(name||headerName(index)),index}));
    const fileRecord={id:crypto.randomUUID(),name:file.name,sheet:data.sheet||"",rows,headers,mapping:defaultColumnMapping(),createdAt:fileCreatedAt,fileDateSource:fileInfoDateSource(file),fileLastModified:file.lastModified||0,sourceBytes:file.size||0,fileToken:data.fileToken||"",rowCount:Number(data.rowCount??data.rows?.length??0),rowsComplete:data.compactResult!==true};
  ...
  async function loadDdioFile(input,sourceInput=null){
    ...
    try{
      ...
      const fileCreatedAt=await resolveFileInfoDate(file);
      if(backendAvailable)try{fileRecord=await backendFileRecord(file,fileCreatedAt,progress);}catch(error){if(networkUnavailable(error))setBackendStatus(false,"Автономный HTML-режим · DDIO обрабатывается в браузере");else throw error;}
  ...
  async function analyzeSingleFile(){
    ...
    try{
      ...
      state.files=[{id:crypto.randomUUID(),name:pendingSingleFile.name,role:"primary",rows:[result.headers,...result.rows],headers,mapping:result.mapping,createdAt:fileCreatedAt,fileDateSource:fileInfoDateSource(pendingSingleFile),fileLastModified:pendingSingleFile.lastModified||0,sourceBytes:pendingSingleFile.size||0,fileToken,rowCount:Number(result.rowCount??result.rows?.length??0),rowsComplete:result.rowsComplete!==false,columnDetection:result.detection}];
      await rememberSourceFile(state.files[0],pendingSingleFile);
      clearResultReference();
      ...
      if(result.snapshot?.id){state.snapshots.unshift({id:result.snapshot.id,name:"Single file: "+pendingSingleFile.name,source:pendingSingleFile.name,createdAt:result.snapshot.createdAt||state.lastAnalysis,deviceCount:Number(resultReference.deviceCount||state.devices.length),devices:[],backendStored:true});}
  ...
  function workspaceFileSummary(file){return{id:file.id,name:file.name,role:file.role,createdAt:file.createdAt||"",fileDate:file.fileDate||"",fileDateSource:file.fileDateSource||"",sourceBytes:Number(file.sourceBytes||0),rowCount:Number(file.rowCount??Math.max(0,(file.rows?.length||1)-1)),fileToken:file.fileToken||""};}
  ...
  function sourceFilesPayload(compact=true){
    return state.files.map((file)=>{
      ...
      return {id:file.id,name:file.name,role:file.role,sheet:file.sheet||"",mapping:file.mapping||{},createdAt:file.createdAt||"",fileDateSource:file.fileDateSource||"",sourceBytes:Number(file.sourceBytes||0),fileToken:file.fileToken,rowCount:Number(file.rowCount??Math.max(0,(file.rows?.length||1)-1))};
    });
  }
  function ddioFilePayload(compact=true){
    ...
    if(!file)return null;
    if(!compact||!file.fileToken)return file;
    return {id:file.id,name:file.name,role:"ddio",sheet:file.sheet||"",mapping:file.mapping||{},createdAt:file.createdAt||"",fileDateSource:file.fileDateSource||"",sourceBytes:Number(file.sourceBytes||0),fileToken:file.fileToken,rowCount:Number(file.rowCount??Math.max(0,(file.rows?.length||1)-1))};
  }
  ...
  function renderDeviceDialog(analytics,local,requestedMac){
    ...
    const chronologicalMovements=events.filter((item)=>item.type==="movement"),localHistory=localMacHistoryRows(normalized,appearances,chronologicalMovements),localMovements=localMacMovementRows(normalized,chronologicalMovements);
    ...
    $("#deviceHistoryBody").innerHTML=localMovements.count?localMovements.html:(analytics?.movementRowsHtml||localMovements.html);
  }
  ...
  function localMacMovementRows(mac,loadedRows=null){const normalized=normalize(mac),rows=(loadedRows||(state.movementHistory||[]).filter((item)=>normalize(item.mac||item.macFormatted)===normalized)).slice().sort((a,b)=>String(b.date||b.changedAt||b.changed_at||"").localeCompare(String(a.date||a.changedAt||a.changed_at||"")));return{count:rows.length,html:rows.length?rows.map((item)=>`<tr><td>${esc(formatDisplayDateTime(item.date||item.changedAt||item.changed_at||""))}</td><td>${esc(item.fieldLabel||item.field||item.field_name||"-")}${ddioHistoryBadge(item)}</td><td>${esc(item.before??item.from_value??"-")}</td><td>${esc(item.after??item.to_value??"-")}</td><td>${esc(item.source||"-")}</td></tr>`).join(""):'<tr><td colspan="5" class="empty-state">Локальные изменения параметров MAC отсутствуют.</td></tr>'};}
  ...
  function loadStatisticsPanel(){
    const cacheKey=[finalDashboardSnapshots().length,state.lastAnalysis||"",state.resultSnapshotId||state.resultBrowserSnapshotId||""].join("|");
    if(statisticsPanelPromise&&statisticsPanelPromiseKey===cacheKey)return statisticsPanelPromise;
    statisticsPanelPromiseKey=cacheKey;
    statisticsPanelPromise=api("/statistics/panel").catch((error)=>{if(statisticsPanelPromiseKey===cacheKey){statisticsPanelPromise=null;statisticsPanelPromiseKey="";}throw error;});
    return statisticsPanelPromise;
  }
  async function renderBackendStatistics(){
    const root=$("#backendStatisticsChart"); if(!root)return;
    try{
      const data=await loadStatisticsPanel();
  ...
  function renderAnalysisDashboardPayload(payload={},statusText=""){
    ...
    chart("#analysisVendorChart",distributions.vendors,"Нет данных производителей.");chart("#analysisModelChart",distributions.models,"Нет данных моделей.");chart("#analysisRoomChart",distributions.rooms,"Нет данных помещений.");chart("#analysisSwitchChart",distributions.switches,"Нет данных коммутаторов.");chart("#analysisOuiChart",[["3 байта",Number(metrics.uniqueOui3||0)],["4 байта",Number(metrics.uniqueOui4||0)],["5 байт",Number(metrics.uniqueOui5||0)]],"Нет OUI.");chart("#analysisOui3Chart",distributions.oui3,"Нет OUI 3 байта.");chart("#analysisOui4Chart",distributions.oui4,"Нет OUI 4 байта.");chart("#analysisOui5Chart",distributions.oui5,"Нет OUI 5 байт.");chart("#analysisCoverageChart",coverage,"Нет данных заполненности.");chart("#analysisQualityChart",[["Опознано",known],["Unknown",unknown],["Ошибки",Number(metrics.invalid||0)]].filter((item)=>item[1]>0),"Ошибок качества не найдено.");chart("#analysisMissingRoomVendorChart",distributions.missingRoomVendors,"Все устройства привязаны к помещениям.");chart("#analysisMissingRoomModelChart",distributions.missingRoomModels,"Все устройства привязаны к помещениям.");
  ...
  function compactDashboardDevice(device){
    ...
    return{internalDeviceId:value("internalDeviceId","internal_device_id"),mac:normalize(device.mac||device.macFormatted)||"",vendor:value("vendor"),model:value("model"),deviceType:value("deviceType","device_type","modelType","model_type","type"),ip:value("ip"),address:value("address"),room:value("room"),smartroomId:value("smartroomId","smartroom_id"),switchIp:value("switchIp","switch_ip"),switchPort:value("switchPort","switch_port"),authenticationTime:value("authenticationTime","authentication_time","authTime"),hostname:value("hostname","host_name"),serialNumber:value("serialNumber","serial_number","serial"),deviceId:value("deviceId","device_id"),deviceName:value("deviceName","device_name"),hasConflict:Boolean(device.hasConflict||(device.conflicts||[]).length)};
  }
  function dashboardDeviceIdentity(device){return DeviceIdentity.comparisonKey(compactDashboardDevice(device)||{});}
  function dashboardChangeIdentity(item){return dashboardDeviceIdentity(item.afterDevice||item.beforeDevice||{mac:item.mac,smartroomId:item.smartroomId,deviceId:item.deviceId,serialNumber:item.serialNumber,hostname:item.hostname})||String(item.identity||item.internalDeviceId||"").trim();}
  function dashboardChangeSeverity(type,field,beforeDevice=null,afterDevice=null){
    ...
    if(["vendor","model","deviceType","smartroomId","smartroom_id","switchIp","switchPort","switch_ip","switch_port"].includes(field))return"medium";
    return"low";
  }
  ...
  function mergeReliableChangeSummary(provided={},changes=[]){const derived=summarizeDashboardChanges(changes),result={...(provided||{})};for(const key of ["added","removed","modified","critical","changedRooms","total","fieldChanges"]){const current=Number(result[key]);if(!Number.isFinite(current)||(current===0&&Number(derived[key]||0)>0))result[key]=Number(derived[key]||0);}result.changedRoomValues=(derived.changedRoomValues||[]).length?derived.changedRoomValues:(result.changedRoomValues||[]);return result;}
  ...
  function localDashboardChangeAnalysis(settings=dashboardSettings()){
    ...
    let baselineSnapshotId=pair.baselineId,comparisonSnapshotId=pair.comparisonId,dateFrom=pair.dateFrom,dateTo=pair.dateTo,roomCoverage={totalRooms:0,changedRooms:0,allChangedRoomCount:0,rooms:[]};
    if(snapshotRows.length>=2&&baselineSnapshotId&&comparisonSnapshotId&&baselineSnapshotId!==comparisonSnapshotId){
      ...
      const source=`Final «${baseline.name||baselineSnapshotId}» → Final «${comparison.name||comparisonSnapshotId}»`;
      roomCoverage=window.MacAnalyzerRoomTimeline?.compareFleetRooms?.(baseline.devices||[],comparison.devices||[])||roomCoverage;
      paired.added.forEach((device)=>row(normalize(device.mac||device.macFormatted),date,"added","device","",device.source||"Устройство",source,null,device));
      paired.removed.forEach((device)=>row(normalize(device.mac||device.macFormatted),date,"removed","device",device.source||"Устройство","",source,device,null));
      const fields=["mac","vendor","model","deviceType","ip","address","room","smartroomId","switchIp","switchPort","hostname","serialNumber","deviceId","deviceName"];
  ...
  function browserSnapshotChangeAnalysis(comparison,options=[],settings=dashboardSettings()){
    ...
    return{mode:settings.changeMode||"snapshots",dateFrom:settings.changeDateFrom||"",dateTo:settings.changeDateTo||"",baselineSnapshotId:comparison.baselineSnapshotId,comparisonSnapshotId:comparison.comparisonSnapshotId,baselineDate,comparisonDate,durationMs:dashboardDurationMs(baselineDate,comparisonDate),snapshotOptions:options,summary,roomCoverage:comparison.roomCoverage||{totalRooms:0,changedRooms:0,allChangedRoomCount:0,rooms:[]},changes,groups:groupDashboardChanges(changes),fieldCounts:comparison.fieldCounts||[]};
  }
  function metadataDashboardFleet(options=[],aggregate={}){
    const snapshots=finalDashboardSnapshots(),series=[];
    for(const item of options||[]){const metadata=snapshots.find((snapshot,position)=>dashboardSnapshotId(snapshot,position)===item.id),count=Number(metadata?.deviceCount||0),previous=Number(series.at(-1)?.count||0);series.push({id:item.id,name:item.name,date:item.date,count,delta:series.length?count-previous:0});}
    const latestCount=Number(series.at(-1)?.count||aggregate.devices||0),largest=Math.max(Number(aggregate.uniqueMacs||0),...series.map((item)=>Number(item.count||0)),latestCount);
    return{uniqueAcrossUploads:largest,latestCount,series,metadataOnly:true};
  }
  function refreshBrowserDashboardFleet(cache){
    if(!cache||cache.fleetExact||!BrowserSnapshots?.aggregateSeries)return Promise.resolve(cache);
    const key=cache.fleetCacheKey;if(browserFleetRefreshPromise&&browserFleetRefreshKey===key)return browserFleetRefreshPromise;
    browserFleetRefreshKey=key;
    browserFleetRefreshPromise=BrowserSnapshots.aggregateSeries(cache.options,{vendor:cache.settings.vendor,room:cache.settings.room,query:cache.settings.query,showUnknown:cache.settings.showUnknown}).then((fleet)=>{
      state.dashboardFleetCache={key,value:fleet};save();
      const updated={...cache,fleet,fleetExact:true};
      if(browserDashboardCache?.cacheKey===cache.cacheKey&&$("#analyticsView")?.classList.contains("active"))renderBrowserDashboardCache(updated);
      return updated;
    }).catch(()=>cache).finally(()=>{if(browserFleetRefreshKey===key){browserFleetRefreshPromise=null;browserFleetRefreshKey="";}});
  ...
  async function loadBrowserDashboardCache(settings=dashboardSettings()){
    ...
    return{cacheKey,fleetCacheKey,options,aggregate,filterAggregate,fleet,fleetExact:Boolean(cachedFleet),comparison,changeAnalysis,report,settings:effectiveSettings};
  }
  ...
  function renderDashboardChanges(analysis={}){
    ...
    dashboardChangeAnalysis.groups=groupDashboardChanges(dashboardChangeAnalysis.changes||[]);
    dashboardChangesVisibleLimit=100;
    const provided=dashboardChangeAnalysis.summary&&typeof dashboardChangeAnalysis.summary==="object"?dashboardChangeAnalysis.summary:{};
    dashboardChangeAnalysis.summary=mergeReliableChangeSummary(provided,dashboardChangeAnalysis.changes||[]);
    const summary=dashboardChangeAnalysis.summary||{},set=(selector,value)=>{if($(selector))$(selector).textContent=Number(value||0).toLocaleString("ru-RU");};
    set("#dashboardChangeTotal",summary.total);set("#dashboardCriticalCount",summary.critical);set("#dashboardAddedCount",summary.added);set("#dashboardRemovedCount",summary.removed);set("#dashboardModifiedCount",summary.modified);set("#dashboardChangedRoomMetric",summary.changedRooms);set("#dashboardAllChangedRoomsMetric",dashboardChangeAnalysis.roomCoverage?.allChangedRoomCount);
    const codecGroups=(dashboardChangeAnalysis.groups||[]).filter((group)=>isVideoConferenceDevice(group.beforeDevice)||isVideoConferenceDevice(group.afterDevice));set("#dashboardCodecChangesMetric",codecGroups.length);
    const duration=dashboardChangeAnalysis.durationMs?` · ${dashboardDurationLabel(dashboardChangeAnalysis.durationMs)}`:"",label=dashboardChangeAnalysis.mode==="snapshots"?`Выгрузки: ${dashboardChangeAnalysis.baselineSnapshotId||"-"} → ${dashboardChangeAnalysis.comparisonSnapshotId||"-"}${duration}`:`Период: ${dashboardChangeAnalysis.dateFrom||"-"} → ${dashboardChangeAnalysis.dateTo||"-"}${duration}`;
    if($("#dashboardChangesPeriodLabel"))$("#dashboardChangesPeriodLabel").textContent=label;if($("#dashboardAllChangedRoomsPeriod"))$("#dashboardAllChangedRoomsPeriod").textContent=label;
    syncDashboardChangeControls({...state.dashboardSettings,...dashboardChangeAnalysis},dashboardChangeAnalysis.snapshotOptions||dashboardSnapshotOptions());
    const revision=++dashboardDetailRenderRevision,renderDetails=()=>{if(revision!==dashboardDetailRenderRevision)return;renderDashboardChangesTable();renderDashboardAllChangedRooms();};
    if(typeof window.requestIdleCallback==="function")window.requestIdleCallback(renderDetails,{timeout:180});else requestAnimationFrame(renderDetails);
  }
  function renderDashboardAllChangedRooms(){
    const rows=(dashboardChangeAnalysis.roomCoverage?.rooms||[]).filter((item)=>item.allChanged),body=$("#dashboardAllChangedRoomsBody");
    if(!body)return;
  ...
  function selectDashboardChangeTab(type="all",focus=false){
    ...
    if(severity)severity.value=filters.severity;if(typeFilter)typeFilter.value=filters.type;dashboardChangeTypeFilter=filters.type;dashboardChangesVisibleLimit=100;syncDashboardChangeTabState();renderDashboardChangesTable();
  ...
  function renderDashboardChangesTable(){
    ...
    const allGroups=dashboardChangeAnalysis.groups||groupDashboardChanges(dashboardChangeAnalysis.changes||[]),filtered=allGroups.filter((group)=>!dashboardCodecOnly||isVideoConferenceDevice(group.beforeDevice)||isVideoConferenceDevice(group.afterDevice)).map((group)=>({group,changes:group.changes.filter((item)=>(severity==="all"||item.severity===severity)&&(type==="all"||item.type===type))})).filter(({group,changes})=>changes.length&&(!macQuery||String(group.mac||"").includes(macQuery))&&(!query||[group.macFormatted,group.device.vendor,group.device.model,group.device.deviceType,group.device.ip,group.device.address,group.device.room,group.device.smartroomId,group.device.switchIp,group.source,...changes.flatMap((item)=>[item.fieldLabel,item.before,item.after])].some((value)=>String(value||"").toLowerCase().includes(query)))),groups=filtered.slice(0,dashboardChangesVisibleLimit);
    const rows=groups.map(({group,changes},index)=>{
      const groupId=`dashboard-change-${index}`,device=group.device||{},fields=changes.map((item)=>item.fieldLabel).filter((value,index,all)=>all.indexOf(value)===index).join(", "),visibleSeverity=changes.some((item)=>item.severity==="critical")?"critical":changes.reduce((best,item)=>({critical:4,high:3,medium:2,low:1}[item.severity]||0)>({critical:4,high:3,medium:2,low:1}[best]||0)?item.severity:best,"low");
      const criticalHint=changes.map(ddioHistoryBadge).find(Boolean)||"";
      const visibleTypes=new Set(changes.map((item)=>item.type)),visibleType=visibleTypes.has("modified")?"modified":visibleTypes.has("added")?"added":"removed",deviceLink=group.mac?`<button class="link-button" data-mac="${esc(group.mac)}">${esc(group.macFormatted||group.mac)}</button>`:`<span>${esc(group.macFormatted||group.identity)}</span>`,parent=`<tr class="change-row change-${esc(visibleSeverity)} dashboard-change-parent" data-dashboard-change-group="${groupId}"><td><span class="severity-badge severity-${esc(visibleSeverity)}">${labels[visibleSeverity]||visibleSeverity}</span>${criticalHint}</td><td>${esc(String(group.date||"-").replace("T"," ").slice(0,19))}</td><td><button class="movement-group-toggle" data-toggle-dashboard-change="${groupId}" aria-expanded="false" title="Развернуть изменения">▸</button> ${deviceLink}</td><td>${esc(typeLabels[visibleType]||visibleType)}</td><td>${esc(device.model||"-")}</td><td>${esc(device.address||"-")}</td><td>${esc(device.room||"-")}</td><td>${esc(fields||"Устройство")}</td><td>${esc(group.source||"-")}</td></tr>`;
      const children=changes.map((item)=>`<tr class="dashboard-change-child change-${esc(item.severity)}" data-dashboard-change-child="${groupId}" hidden><td>${ddioHistoryBadge(item)}</td><td></td><td></td><td>${esc(typeLabels[item.type]||item.type)}</td><td colspan="3">${esc(item.fieldLabel)}</td><td><span class="change-before">${esc(item.before)}</span> → <span class="change-after">${esc(item.after)}</span></td><td>${esc(item.source||group.source||"-")}</td></tr>`).join("");
      return parent+children;
    }).join("");
    const remaining=Math.max(0,filtered.length-groups.length),loadMore=remaining?`<tr><td colspan="9" class="empty-state"><button type="button" class="secondary" data-load-more-dashboard-changes>Показать ещё ${Math.min(100,remaining).toLocaleString("ru-RU")} · осталось ${remaining.toLocaleString("ru-RU")}</button></td></tr>`:"";
    $("#dashboardChangesBody").innerHTML=groups.length?rows+loadMore:'<tr><td colspan="9" class="empty-state">Изменений по выбранным условиям не найдено.</td></tr>';
  }
  function isVideoConferenceDevice(device){return String(device?.deviceType||device?.device_type||device?.modelType||device?.model_type||device?.type||"").trim().toLowerCase()==="video conference";}
  function showDashboardChangesDialog(type="all",codecOnly=false){dashboardCodecOnly=Boolean(codecOnly);const label=$("#dashboardChangesPeriodLabel");if(label){const base=String(label.textContent||"").replace(/^Кодеки Video Conference · /,"");label.textContent=dashboardCodecOnly?`Кодеки Video Conference · ${base}`:base;}selectDashboardChangeTab(type);const dialog=$("#dashboardChangesDialog");if(dialog&&!dialog.open)dialog.showModal();}
  ...
  function renderDashboardStatus(payload={}){
    ...
    draw("#dashboardDynamicsChart",charts.dynamics,"Нет сохранённых финальных выгрузок.");draw("#dashboardStatusVendorChart",charts.vendors,"Нет данных производителей.");draw("#dashboardFieldChangesChart",charts.fields,"Изменения полей не найдены.");draw("#dashboardMissingChart",charts.missing,"Отсутствовавших устройств нет.");draw("#dashboardMissingRoomVendorChart",charts.missingRoomVendors,"Все устройства привязаны к помещениям.");draw("#dashboardMissingRoomModelChart",charts.missingRoomModels,"Все устройства привязаны к помещениям.");
  ...
  function presenceChurnKey(){return finalDashboardSnapshots().map((snapshot,index)=>`${dashboardSnapshotId(snapshot,index)}:${snapshot.deviceCount??(snapshot.devices||[]).length}`).join("|");}
  async function calculatePresenceChurn(){
    const snapshots=finalDashboardSnapshots(),key=presenceChurnKey();if(presenceChurnCache.key===key)return presenceChurnCache.data;
    let data=null;
    if(snapshots.some((snapshot)=>snapshot.browserStored)&&BrowserSnapshots?.streamSnapshot){
      const tracker=PresenceChurn.createTracker();
      for(const snapshot of snapshots){
        if((snapshot.devices||[]).length){tracker.observe(snapshot,snapshot.devices);continue;}
        if(!snapshot.browserStored)continue;
        const rows=[];await BrowserSnapshots.streamSnapshot(dashboardSnapshotId(snapshot,0),async(kind,chunk)=>{if(kind==="device")for(const device of chunk)rows.push(compactDashboardDevice(device));});tracker.observe(snapshot,rows);await new Promise((resolve)=>setTimeout(resolve,0));
      }
      data=tracker.result({limit:500});
    }else if(backendAvailable){data=await api("/analytics/presence-churn",{method:"POST",body:JSON.stringify({snapshots,limit:500})});}
    else data=PresenceChurn.analyze(snapshots,{limit:500});
    presenceChurnCache={key,data:data||{snapshotCount:snapshots.length,total:0,items:[]}};return presenceChurnCache.data;
  }
  function renderPresenceChurn(data){
    const items=data?.items||[],chart=$("#presenceChurnChart"),summary=$("#presenceChurnSummary");if(summary)summary.textContent=`${Number(data?.total||0).toLocaleString("ru-RU")} MAC · ${Number(data?.snapshotCount||0).toLocaleString("ru-RU")} финальных выгрузок`;if(chart)chart.innerHTML=localChartHtml(items.slice(0,12).map((item)=>[item.macFormatted||formatMac(item.mac),Number(item.transitionCount||0)]),"MAC с повторным исчезновением и возвращением не найдены.");
  }
  function renderPresenceChurnDialog(){const data=presenceChurnCache.data||{},query=String($("#presenceChurnSearch")?.value||"").trim().toLowerCase(),rows=(data.items||[]).filter((item)=>!query||Object.values(item).join(" ").toLowerCase().includes(query));$("#presenceChurnBody").innerHTML=rows.length?rows.map((item)=>`<tr><td><button class="link-button" type="button" data-mac="${esc(item.mac)}">${esc(item.macFormatted||formatMac(item.mac))}</button></td><td>${esc(item.vendor||"—")}</td><td>${esc(item.model||"—")}</td><td>${esc(item.ip||"—")}</td><td>${esc(item.room||item.smartroomId||"—")}</td><td>${Number(item.presentCount||0)}</td><td>${Number(item.disappearances||0)}</td><td>${Number(item.reappearances||0)}</td><td><strong>${Number(item.transitionCount||0)}</strong></td></tr>`).join(""):'<tr><td colspan="9" class="empty-state">MAC с повторным исчезновением и возвращением не найдены.</td></tr>';}
  async function refreshPresenceChurn(){try{renderPresenceChurn(await calculatePresenceChurn());}catch(error){if($("#presenceChurnChart"))$("#presenceChurnChart").innerHTML=`<p class="muted">Не удалось рассчитать: ${esc(error.message)}</p>`;}}
  async function openPresenceChurn(){await refreshPresenceChurn();renderPresenceChurnDialog();$("#presenceChurnDialog")?.showModal();}
    ...
    if(browserOnlyMode&&state.resultBrowserSnapshotId&&BrowserSnapshots?.aggregate){
      try{
        ...
        if(revision===analyticsRenderRevision&&renderBrowserDashboardCache(cache)){
          ...
          void refreshBrowserDashboardFleet(cache);
          scheduleAnalyticsSecondaryPanels(revision,dashboardDevices());
  ...
  async function runGlobalSearch(){
    ...
    const body=$("#globalSearchResults"),status=$("#globalSearchStatus");body.innerHTML='<tr><td colspan="5" class="empty-state">Поиск по накопленному реестру…</td></tr>';$("#globalSearchDialog")?.showModal();
    let rows=[];
    try{
      if(backendAvailable){const data=await api("/database/search?query="+encodeURIComponent(query)+"&limit=500"),unique=new Map();for(const item of data.results||[]){if(!["device","vendorModel"].includes(item.type))continue;const itemMac=normalize(item.mac);if(!itemMac)continue;const previous=unique.get(itemMac);if(previous&&previous.type==="device"&&item.type!=="device")continue;unique.set(itemMac,{type:item.type,mac:itemMac,macFormatted:formatMac(itemMac),vendorModel:item.subtitle||item.title,ip:"",room:"",source:item.source});}rows=[...unique.values()].slice(0,100);}
      if(!rows.length&&BrowserSnapshots?.streamDeviceHistory){const needle=query.toLowerCase(),seen=new Set();await BrowserSnapshots.streamDeviceHistory(async(chunk)=>{for(const item of chunk){if(rows.length>=100)break;const text=Object.values(item||{}).join(" ").toLowerCase(),itemMac=normalize(item.mac);if(!itemMac||seen.has(itemMac)||!text.includes(needle))continue;seen.add(itemMac);rows.push({mac:itemMac,macFormatted:formatMac(itemMac),vendorModel:[item.vendor,item.model].filter(Boolean).join(" · "),ip:item.ip||"",room:[item.room,item.address].filter(Boolean).join(" · "),source:item.source||"IndexedDB"});}});}
      status.textContent=`Найдено: ${rows.length.toLocaleString("ru-RU")} · поиск выполнен по всем сохранённым Final.`;
      body.innerHTML=rows.length?rows.map((item)=>`<tr><td><button class="link-button" type="button" data-global-search-mac="${esc(item.mac)}">${esc(item.macFormatted||formatMac(item.mac))}</button></td><td>${esc(item.vendorModel||"—")}</td><td>${esc(item.ip||"—")}</td><td>${esc(item.room||"—")}</td><td>${esc(item.source||"—")}</td></tr>`).join(""):'<tr><td colspan="5" class="empty-state">Во всех сохранённых выгрузках ничего не найдено.</td></tr>';
    }catch(error){body.innerHTML=`<tr><td colspan="5" class="empty-state">Поиск не выполнен: ${esc(error.message)}</td></tr>`;}
  }
  ...
  async function exportAllDevicesCsv(){
    if(!BrowserSnapshots?.countDeviceHistory||!BrowserSnapshots?.streamDeviceHistory)throw new Error("Накопительная база устройств недоступна");
    const columns=exportColumns(),parts=["\uFEFF",localCsvLine(columns.map((column)=>column.title))+"\r\n"],processId=beginProcess("Все устройства CSV","Потоковая выгрузка накопительной базы",5);let count=0;
    try{
      await streamAllDeviceRows(async(rows)=>{let batch="";for(const device of rows||[]){batch+=localCsvLine(columns.map((column)=>exportCell(device,column.key,count)))+"\r\n";count+=1;}if(batch)parts.push(batch);updateProcess(processId,Math.min(95,5+Math.round(count/Math.max(count,Number(state.resultDeviceCount||0),1)*90)),`Подготовлено ${count.toLocaleString("ru-RU")} устройств`);await new Promise((resolve)=>setTimeout(resolve,0));});
      if(!count)throw new Error("В накопительной базе пока нет устройств");const date=new Date().toISOString().replace(/[:.]/g,"-").slice(0,19);deliverDownload(`mac-analyzer-all-devices-${date}.csv`,new Blob(parts,{type:"text/csv;charset=utf-8"}));finishProcess(processId,`Выгружено устройств: ${count.toLocaleString("ru-RU")}`);toast("Полный CSV накопительной базы сохранён.");return count;
    }catch(error){failProcess(processId,error);throw error;}
  }
  function exportDashboardChangesCsv(){const changes=dashboardChangeAnalysis?.changes||[];if(!changes.length){toast("Для выбранного периода изменений нет.");return false;}const headers=["Дата","MAC-адрес","Тип","Критичность","Поле","Было","Стало","Производитель","Модель","Тип устройства","IP устройства","Адрес","Помещение","Smartroom ID","IP коммутатора","Порт","Источник"],rows=changes.map((item)=>{const device=item.afterDevice||item.beforeDevice||{};return[item.date||item.changedAt||"",formatMac(item.mac)||item.mac||"",item.typeLabel||item.type||"",item.severity||"",item.fieldLabel||item.field||"",item.before??"",item.after??"",device.vendor||"",device.model||"",device.deviceType||device.device_type||"",device.ip||"",device.address||"",device.room||"",device.smartroomId||device.smartroom_id||"",device.switchIp||device.switch_ip||"",device.switchPort||device.switch_port||"",item.source||""];});const content="\uFEFF"+[headers,...rows].map((row)=>localCsvLine(row)).join("\r\n"),date=new Date().toISOString().replace(/[:.]/g,"-").slice(0,19);download(`mac-analyzer-changes-${date}.csv`,content,"text/csv");toast(`Выгружено изменений: ${rows.length.toLocaleString("ru-RU")}.`);return true;}
  ...
  async function exportFullWorkbook(){
    ...
    try{
      ...
      const inventoryDeviceCount=BrowserSnapshots?.countDeviceHistory?await BrowserSnapshots.countDeviceHistory():currentDeviceCount();
      const report=await FullXlsxReport.buildReport({
        state:{...state,reportChanges:dashboardChangeAnalysis?.changes||[]},
  ...
  function localSystemDiagnostics(){
    const checks=[
      ["Frontend","Основной контроллер",Boolean(window.MacAnalyzerAppBootstrapped),window.MacAnalyzerAppReady?"app.js загружен и состояние восстановлено":"app.js загружен; восстановление базы продолжается"],
  ...
  $("#batchEnrichmentPlan")?.addEventListener("click",(event)=>{const button=event.target.closest("[data-batch-review-mapping]");if(button)reviewBatchMappings(button.dataset.batchReviewMapping);});
  $("#applyBatchMappingReviewButton")?.addEventListener("click",applyBatchMappingReview);
  $("#cancelBatchMappingReviewButton")?.addEventListener("click",()=>{$("#batchMappingReviewDialog")?.close();batchMappingReviewContext=null;});
  $("#closeBatchMappingReviewDialog")?.addEventListener("click",()=>{$("#batchMappingReviewDialog")?.close();batchMappingReviewContext=null;});
  ...
  $("#exportAllDevicesCsvButton").addEventListener("click",async()=>{try{await exportAllDevicesCsv();}catch(error){UiFeedback?.showError(error);toast(error.message||"Не удалось выгрузить все устройства в CSV.");}});
  $("#exportDashboardChangesCsvButton").addEventListener("click",exportDashboardChangesCsv);
  ...
  $("#closeGlobalSearchDialog")?.addEventListener("click",()=>$("#globalSearchDialog")?.close());
  $("#globalSearchResults")?.addEventListener("click",(event)=>{const button=event.target.closest("[data-global-search-mac]");if(button)showDevice(button.dataset.globalSearchMac);});
  ...
  $("#createArchiveButton")?.addEventListener("click",async()=>{try{await writeBrowserArchive($("#archiveNameInput")?.value);if($("#archiveNameInput"))$("#archiveNameInput").value="";await renderArchiveList();toast("Архив состояния создан.");}catch(error){toast(error.message);}});
  $("#archiveList")?.addEventListener("click",async(event)=>{const load=event.target.closest("[data-load-archive]"),exportButton=event.target.closest("[data-export-archive]"),remove=event.target.closest("[data-delete-archive]");try{if(load)await loadBrowserArchive(load.dataset.loadArchive);if(exportButton)await exportBrowserArchive(exportButton.dataset.exportArchive);if(remove&&confirm("Удалить выбранный архив? Текущая база и Final не будут удалены.")){await deleteBrowserArchive(remove.dataset.deleteArchive);await renderArchiveList();toast("Архив удалён.");}}catch(error){toast(error.message);}});
  $("#importArchiveInput")?.addEventListener("change",async(event)=>{try{await importBrowserArchiveFile(event.target.files?.[0]);}catch(error){UiFeedback?.showError(error);toast(error.message);}finally{event.target.value="";}});
  ...
  $("#dashboardRoomFilter").addEventListener("change",()=>renderAnalytics({force:true}));
  $("#dashboardStatusFilter").addEventListener("change",()=>renderAnalytics({force:true}));
  $("#refreshDashboardButton").addEventListener("click",()=>renderAnalytics({force:true}));
  $$("[data-dashboard-status]").forEach((button)=>button.addEventListener("click",async()=>{const selected=button.dataset.dashboardStatus||"all",status=$("#dashboardStatusFilter");if(status)status.value=selected;await renderAnalytics({force:true});if(selected==="all")showDashboardDynamicsDialog();else if(selected==="changed")showDashboardChangesDialog("modified");else if(selected==="missing")showDashboardChangesDialog("removed");}));
  ...
  $("#applyDashboardChangeRangeButton").addEventListener("click",async()=>{readDashboardChangeControls();await renderAnalytics({force:true});showDashboardChangesDialog();});
  $("#openDashboardChangesButton").addEventListener("click",()=>showDashboardChangesDialog("all"));
  $("#dashboardChangeSeverityFilter").addEventListener("change",()=>{dashboardChangesVisibleLimit=100;syncDashboardChangeTabState();renderDashboardChangesTable();});
  $("#dashboardChangeTypeFilter").addEventListener("change",()=>{dashboardChangesVisibleLimit=100;syncDashboardChangeTabState();renderDashboardChangesTable();});
  $("#dashboardChangeSearch").addEventListener("input",debounce(()=>{dashboardChangesVisibleLimit=100;renderDashboardChangesTable();},180));
  $("#dashboardChangeMacSearch").addEventListener("input",debounce(()=>{dashboardChangesVisibleLimit=100;renderDashboardChangesTable();},120));
  ...
  $("#dashboardChangesBody").addEventListener("click",(event)=>{const loadMore=event.target.closest("[data-load-more-dashboard-changes]");if(loadMore){dashboardChangesVisibleLimit+=100;renderDashboardChangesTable();return;}const toggle=event.target.closest("[data-toggle-dashboard-change]");if(toggle){const id=toggle.dataset.toggleDashboardChange,expanded=toggle.getAttribute("aria-expanded")==="true";toggle.setAttribute("aria-expanded",expanded?"false":"true");toggle.textContent=expanded?"▸":"▾";$$(`[data-dashboard-change-child="${id}"]`).forEach((row)=>{row.hidden=expanded;});return;}const macButton=event.target.closest("[data-mac]");if(macButton)showDevice(macButton.dataset.mac);});
  ...
  $("#closePresenceChurnDialog")?.addEventListener("click",()=>$("#presenceChurnDialog")?.close());
  $("#presenceChurnSearch")?.addEventListener("input",debounce(renderPresenceChurnDialog,120));
  $("#presenceChurnBody")?.addEventListener("click",(event)=>{const button=event.target.closest("[data-mac]");if(button)showDevice(button.dataset.mac);});
