(() => {
  ...
  function normalizeDashboardSettings(settings={}){
    const defaults={query:"",vendor:"",room:"",status:"all",chartLimit:8,showUnknown:true,visibleCards:{total:true,changed:true,missing:true,unchanged:true,vendors:true,rooms:true,changedRooms:true,allChangedRooms:true},visibleCharts:{dynamics:true,vendors:true,fields:true,missing:true},autoRefresh:true,refreshInterval:60,changeMode:"snapshots",changeDateFrom:"",changeDateTo:"",baselineSnapshotId:"",comparisonSnapshotId:""},source=settings&&typeof settings==="object"?settings:{};
  ...
  function renderDashboardAllChangedRooms(){
    ...
    if(!body)return;
    body.innerHTML=rows.length?rows.map((item)=>`<tr><td>${esc(item.smartroomId||"—")}</td><td>${esc(item.tb||"—")}</td><td>${esc(item.city||"—")}</td><td>${esc(item.site||"—")}</td><td>${esc(item.floor||"—")}</td><td>${esc(item.room||item.smartroomId||"—")}</td><td><strong>${Number(item.changed||0).toLocaleString("ru-RU")} / ${Number(item.total||0).toLocaleString("ru-RU")}</strong></td><td>${Number(item.added||0).toLocaleString("ru-RU")}</td><td>${Number(item.removed||0).toLocaleString("ru-RU")}</td><td>${Number(item.modified||0).toLocaleString("ru-RU")}</td></tr>`).join(""):'<tr><td colspan="10" class="empty-state">Нет переговорных, где между выбранными Final изменилось всё оборудование.</td></tr>';
  }
