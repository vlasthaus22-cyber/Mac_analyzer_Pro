(() => {
  ...
  const defaultColumnWidths = {macFormatted:180,oui:110,vendor:150,model:150,ip:130,address:200,room:120,smartroomId:140,switchIp:150,switchPort:100,authenticationTime:210,hostname:160,serialNumber:160,deviceId:150,deviceName:180,source:150};
  ...
  function localSearchText(item){if(!item||typeof item!=="object")return "";if(localSearchTextCache.has(item))return localSearchTextCache.get(item);const text=[item.mac,item.macFormatted,item.vendor,item.model,item.ip,item.address,item.room,item.smartroomId,item.switchIp,item.switchPort,item.authenticationTime,item.hostname,item.serialNumber,item.deviceId,item.deviceName,item.source].map((value)=>String(value||"")).join(" ").toLowerCase();localSearchTextCache.set(item,text);return text;}
  ...
  function normalizeDashboardSettings(settings={}){
    const defaults={query:"",vendor:"",room:"",status:"all",chartLimit:8,showUnknown:true,visibleCards:{total:true,changed:true,missing:true,unchanged:true,vendors:true,rooms:true,missingRoom:true,changedRooms:true,allChangedRooms:true},visibleCharts:{dynamics:true,vendors:true,fields:true,missing:true,presenceChurn:true},autoRefresh:true,refreshInterval:60,changeMode:"snapshots",changeDateFrom:"",changeDateTo:"",baselineSnapshotId:"",comparisonSnapshotId:""},source=settings&&typeof settings==="object"?settings:{};
  ...
  function renderDashboardChanges(analysis={}){
    ...
    const derived=summarizeDashboardChanges(dashboardChangeAnalysis.changes||[]),provided=dashboardChangeAnalysis.summary&&typeof dashboardChangeAnalysis.summary==="object"?dashboardChangeAnalysis.summary:{};
    dashboardChangeAnalysis.summary={...derived,...provided,changedRoomValues:provided.changedRoomValues||derived.changedRoomValues};
  ...
  function renderDashboardChangesTable(){
    ...
    const allGroups=dashboardChangeAnalysis.groups||groupDashboardChanges(dashboardChangeAnalysis.changes||[]),filtered=allGroups.map((group)=>({group,changes:group.changes.filter((item)=>(severity==="all"||item.severity===severity)&&(type==="all"||item.type===type))})).filter(({group,changes})=>changes.length&&(!macQuery||String(group.mac||"").includes(macQuery))&&(!query||[group.macFormatted,group.device.vendor,group.device.model,group.device.ip,group.device.address,group.device.room,group.device.smartroomId,group.device.switchIp,group.source,...changes.flatMap((item)=>[item.fieldLabel,item.before,item.after])].some((value)=>String(value||"").toLowerCase().includes(query)))),groups=filtered.slice(0,dashboardChangesVisibleLimit);
  ...
  async function calculatePresenceChurn(){
    ...
    if(snapshots.some((snapshot)=>snapshot.browserStored)&&BrowserSnapshots?.streamSnapshot){
      ...
      for(const snapshot of snapshots){
        ...
        const rows=[];await BrowserSnapshots.streamSnapshot(dashboardSnapshotId(snapshot,0),async(kind,chunk)=>{if(kind==="devices")for(const device of chunk)rows.push(compactDashboardDevice(device));});tracker.observe(snapshot,rows);
      }
  ...
  $("#archiveList")?.addEventListener("click",async(event)=>{const load=event.target.closest("[data-load-archive]"),remove=event.target.closest("[data-delete-archive]");try{if(load)await loadBrowserArchive(load.dataset.loadArchive);if(remove&&confirm("Удалить выбранный архив? Текущая база и Final не будут удалены.")){await deleteBrowserArchive(remove.dataset.deleteArchive);await renderArchiveList();toast("Архив удалён.");}}catch(error){toast(error.message);}});
