(() => {
  ...
  const source = `
    ...
    const ips = value => Array.from(new Set((Array.isArray(value) ? value : text(value).split(/[;,|\\s]+/)).map(text).filter(Boolean)));
    const compact = row => ({
      mac: mac(row.mac || row.macFormatted || row.physical_address),
      smartroomId: text(row.smartroomId || row.smartroom_id || row.Smartroom_ID),
      room: text(row.room || row.room_name || row.Room), address: text(row.address || row.physicalAddress),
      switchIp: text(row.switchIp || row.switch_ip || row.ip_switch), switchPort: text(row.switchPort || row.switch_port || row.port),
      ip: text(row.ip || row.deviceIp || row.device_ip), vendor: text(row.vendor || row.manufacturer), model: text(row.model),
    ...
    function ingest(id, rows) {
      ...
      for (const raw of rows || []) {
        ...
        if (!row.smartroomId) {
          s.skipped += 1;
          console.warn('[Smartroom] Запись пропущена: Smartroom ID пустой или null', raw);
          continue;
        }
    ...
    function endSnapshot(id) {
      ...
      const meta = s.pending.meta, next = s.pending.next, date = text(meta.createdAt || meta.date || new Date().toISOString());
      ...
      for (const [identity, row] of next) {
        ...
        if (before.switchIp && row.switchIp && before.switchIp !== row.switchIp) {
          changed += 1;
          const overlay = s.options.ddioOverlay?.[row.mac] || {};
    ...
    function finish(id) {
      ...
      for (const [roomKey, events] of s.rooms) {
        ...
        rooms.push({ smartroomId: latest.devices[0]?.smartroomId || prior.devices[0]?.smartroomId || roomKey.replace(/^MAC:/, ''), room: latest.devices[0]?.room || prior.devices[0]?.room || '', address: latest.devices[0]?.address || prior.devices[0]?.address || '', devices: latest.devices, missing, history: events });
      }
  ...
  async function build(snapshots, options = {}) {
    ...
    const ordered = Array.from(snapshots || []).sort((a, b) => (Date.parse(a.createdAt || a.date || "") || 0) - (Date.parse(b.createdAt || b.date || "") || 0));
