  ...
  const source = `
    ...
    function ingest(id, rows) {
      ...
      for (const raw of rows || []) {
        ...
        const identity = row.mac ? 'mac:' + row.mac : row.deviceId ? 'device:' + row.deviceId.toLowerCase() : 'room:' + row.smartroomId;
    ...
    function endSnapshot(id) {
      ...
      for (const [identity, row] of next) {
        ...
        const changedFields = ['vendor', 'model', 'ip', 'address', 'room', 'smartroomId', 'switchIp', 'switchPort', 'deviceId'];
        if (changedFields.some(field => before[field] && row[field] && before[field] !== row[field])) changed += 1;
